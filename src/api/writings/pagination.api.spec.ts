...
describe("PaginationApi", () => {
  const page = 1;
  let paginationApi: PaginationApi;

  beforeEach(() => {
    const httpMethod = (mockHttpMethod as unknown) as HttpMethod;
    paginationApi = new PaginationApi(httpMethod);
  });

  it("API 호출 시 page 파라미터가 쿼리스트링으로 추가 된다", async () => {
    await paginationApi.request(page);
  ...
  it("API 응답이 성공할 경우 전달 받은 데이터를 반환 한다", async () => {
    ...
    const actual = await paginationApi.request(page);
