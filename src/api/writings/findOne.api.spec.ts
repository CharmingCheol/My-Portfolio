...
import FindOneApi from "./findOne.api";
...
describe("FindOneApi", () => {
  const id = "id";
  let findOneApi: FindOneApi;

  beforeEach(() => {
    const httpMethod = (mockHttpMethod as unknown) as HttpMethod;
    findOneApi = new FindOneApi(httpMethod);
  });

  it("전달 된 id가 API 주소에 포함 된다", async () => {
    await findOneApi.request(id);
  ...
  it("API 응답이 성공할 경우 전달 받은 데이터를 반환 한다", async () => {
    ...
    const actual = await findOneApi.request(id);
